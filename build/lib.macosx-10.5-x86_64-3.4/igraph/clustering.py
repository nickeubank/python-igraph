...
class Clustering(object):
    ...
    def summary(self, verbosity=0, width=None):
        ...
        """
        out = StringIO()
        print("Clustering with %d elements and %d clusters" % \
                (len(self._membership), len(self)), file=out)
...
class Dendrogram(object):
    ...
    def summary(self, verbosity=0, max_leaf_count=40):
        ...
        print("Dendrogram, %d elements, %d merges" % \
                (self._nitems, self._nmerges), file=out)
        ...
        while midx < self._nmerges:
            ...
            print(char_array.tostring(), file=out)
...
class VertexCover(Cover):
    ...
    def __init__(self, graph, clusters = None):
        ...
        """
        if clusters is None:
            clusters = [list(range(graph.vcount()))]
...
class CohesiveBlocks(VertexCover):
    ...
    def hierarchy(self):
        ...
        edges = [pair for pair in zip(self._parent, range(len(self)))
...
def _prepare_community_comparison(comm1, comm2, remove_none=False):
    ...
    if remove_none and (None in vec1 or None in vec2):
        idxs_to_remove = [i for i in range(len(vec1)) \
